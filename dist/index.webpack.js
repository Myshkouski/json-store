...
var _fileStore = __webpack_require__(3);
...
var _parseJson = __webpack_require__(13);

var _parseJson2 = _interopRequireDefault(_parseJson);

var _jsonPatch = __webpack_require__(10);

var _jsonPatch2 = _interopRequireDefault(_jsonPatch);

var _jsonPointer = __webpack_require__(11);

var _jsonPointer2 = _interopRequireDefault(_jsonPointer);

var _emitSafe = __webpack_require__(1);

var _emitSafe2 = _interopRequireDefault(_emitSafe);
...
var JSONStore = function (_FileStore) {
  ...
  function JSONStore(options) {
    ...
    var _this = _possibleConstructorReturn(this, (JSONStore.__proto__ || Object.getPrototypeOf(JSONStore)).call(this, Object.assign({}, options, {
      serialize: function serialize(obj) {
        return JSON.stringify(obj);
      },
      deserialize: function deserialize(string) {
        return (0, _parseJson2.default)(string);
      }
    })));
  ...
  _createClass(JSONStore, [{
    ...
    value: function patch(ops) {
      ...
      _jsonPatch2.default.apply(this[symbols.STORE], ops);

      if (this._options.sync) {
        this.save().catch(function (err) {
          return _emitSafe2.default.call(_this2, 'error', err);
        });
      }
...
function _sync(task) {
  ...
  this._syncing = true;
  var p = this[symbols.SCHEDULE].deferred(function () {
    return new Promise(task).then(function () {
      _this._syncing = false;
      return _emitSafe2.default.call(_this, 'synced');
    }).catch(function (err) {
      _this._syncing = false;
      throw err;
    });
...
var FileStore = function (_EventEmitter) {
  ...
  function FileStore(options) {
    ...
    var _this2 = _possibleConstructorReturn(this, (FileStore.__proto__ || Object.getPrototypeOf(FileStore)).call(this));

    _this2[symbols.STORE] = null;
    _this2._watcher = null;

    _this2[symbols.SCHEDULE] = new _schedule2.default();
    //this[symbols.SCHEDULE].on('error', err => this.emit('error', err))

    _this2[symbols.SERIALIZE] = options.serialize; //.bind(this)
    _this2[symbols.DESERIALIZE] = options.deserialize; //.bind(this)
    ...
    if (options.filename) {
      _this2._options.filename = (0, _path.resolve)(process.cwd(), options.filename);
      _this2._pesisted = true;
      _this2._syncing = false;
    }

    if (_this2._pesisted && options.sync) {
      _this2.load().catch(function (err) {
        return _emitSafe2.default.call(_this2, 'error', err);
      });
    }
    return _this2;
  ...
  _createClass(FileStore, [{
    ...
    value: function load() {
      ...
      var filename = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this._options.filename;

      return _sync.call(this, function (done, fail) {
        (0, _fs.readFile)(filename, function (err, buffer) {
          if (err) {
            fail(err);
          }

          done((0, _lodash2.default)(_this3[symbols.STORE], _this3[symbols.DESERIALIZE](buffer.toString())));
        });
      });
    }
  }, {
    ...
    value: function save() {
      ...
      return _sync.call(this, function (done, fail) {
        (0, _fs.writeFile)(filename, _this4[symbols.SERIALIZE](_this4[symbols.STORE]), function (err) {
          err ? fail(err) : done();
        });
      });
    }
    ...
    value: function watch() {
      ...
      this._watcher = (0, _fs.watch)(this._options.filename, function (event, filename) {
        if (event == 'rename') {
          _this5._options.filename = filename;

          _this5.unwatch();
          _this5.load().catch(function (err) {
            return _emitSafe2.default.call(_this5, 'error', err);
          }).then(function () {
            return _this5.watch();
          });
...
var Schedule = function () {
  ...
  _createClass(Schedule, [{
    ...
    value: function immediate(task) {
      var p = Promise.all([this[PENDING], Promise.resolve(task())]);

      this[PENDING] = p.catch(function (err) {});
...
var Uni = __webpack_require__(18)
